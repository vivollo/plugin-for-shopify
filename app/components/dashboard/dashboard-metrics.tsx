...
export const DashboardMetrics = ({ reports }: DashboardMetricsProps) => {
	const kpis = useMemo(() => {
		const totalCurrent = safeNumber(
			reports.total_conversations.current?.[0]?.value,
		);
		const totalPrev = safeNumber(
			reports.total_conversations.previous?.[0]?.value,
		);
