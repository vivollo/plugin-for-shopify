...
export function CollectionListItem({
	...
	const collectionGroupId = useMemo(
		() => String(collectionGroup.id),
		[collectionGroup.id],
	);
	...
	const handleSync = useCallback(() => {
		...
		fetcher.submit(
			{ collectionGroupId },
			{ method: "post", action: "/api/sync-collection" },
		);
	...
	}, [collectionGroupId, fetcher.data, fetcher.state]);
			...
			<s-stack gap="base">
				...
				>
					<s-stack>
						...
						<s-heading>
							{canSync
								? `${collectionGroup.default_documents_count} documents`
								: "Syncing..."}
						</s-heading>
					</s-stack>
...
CollectionListItem.Skeleton = function CollectionListItemSkeleton() {
	return (
		<s-stack alignItems="center" gap="base" padding="large">
			<s-spinner accessibilityLabel="Loading collections" size="large" />
			<s-text>Loading collections...</s-text>
		</s-stack>
	);
};
