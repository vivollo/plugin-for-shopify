...
export class WidgetService {
	...
	static async publishWidget(sessionId: string, admin: AdminApiContext) {
		...
		const value = this.buildConfigValue(
			session.channelId,
			session.tenantName,
			settings,
		);
	...
	static async getOnboardingStatus(
		...
		return {
			...
			hasPublishedWidget:
				Boolean(session.hasPublishedWidget) &&
	...
	private static async upsertShopMetafield(
		admin: AdminApiContext,
		ownerId: string,
		value: string,
	) {
	...
	private static async getMainThemeSettingsDataJson(
      ...
      query GetMainThemeAndSettingsData($filenames: [String!]!) {
		...
		const file = mainTheme?.files?.nodes?.find(
			(f) => f.filename === this.THEME_SETTINGS_FILE,
		);

		return file?.body?.content ?? null;
	}

	private static detectAppEmbedStatus(
		settingsData: any,
		apiKey: string,
